...
static void Timely_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Timely.getBinding requires 1 argument: binding");
...
static void Timely_init(Local<Object> exports, Local<Context> context)
{
...
static void Timely_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
...
static titanium::bindings::BindEntry TimelyBinding = {
	"ti.mely",
	Timely_init,
	Timely_dispose
};
...
Java_ti_mely_TimelyBootstrap_nativeBootstrap
